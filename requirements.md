...
Owner CSS and marker attributes arrive only through immutable compiled Template
metadata and exact mount/property bindings. React owns no public CSS authoring
API, object serializer, pseudo vocabulary or stylesheet factory.

`bindStyle` is an internal compiled transport. It accepts CSS strings,
`false`/`null`/`undefined` and nested arrays of those values generated by
Template, flattens them in order and writes one inline `style` attribute in the
same transaction as other host patches. Runtime objects, numbers, tokens and
arbitrary author values fail closed. Governed source authors use the global
Template `css`` intrinsic; JSX `class`, `className`, style objects and author
arrays fail at the Template compiler boundary.
...
`ComponentRoot.readStyleSheets()` exposes one immutable root-scoped snapshot
`{revision, styleSheets}` in first-adoption order. Its records are the exact
Template metadata objects, including optional source metadata; React never
reconstructs them from only id/cssText. Snapshot identity remains stable until
that root successfully adopts a new unique id. Roots in one Document never mix
their snapshots even though the Document registry independently deduplicates
their equal records. Failed adoption restores the exact previous snapshot;
after `unmount()` the root API is inaccessible and its internal snapshot is
cleared without scanning component instances.
