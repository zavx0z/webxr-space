...
import {CachedText, Object3D, TrueTypeFont, Vector3, type Color} from "@engine/core"
import {createUiPolylineStrokeGeometry} from "@layout/core/polyline"
import {UiSurface, Z} from "@layout/core/surface"
import {uiIcons} from "@ui/elements/icons"
import {projectFontBytes} from "./test/project-font.ts"
import {
  SOCKET_KINDS,
  SOCKET_PRESETS,
  SOCKET_SHAPES,
  SOCKET_VISUAL_POLICY,
  nodeRenderer,
  socketPreset,
  socketRenderer,
  socketVisualBounds,
...
import {parameterRenderer} from "./parameter.ts"
...
beforeAll(async () => {
  const bytes = await projectFontBytes()
  projectFont = new TrueTypeFont(bytes)
})

describe("Node view presets", () => {
  test("publishes the complete first socket catalog and eight source shapes", () => {
    ...
    expect(socketPreset("float").defaultFieldKind).toBe("number")
    expect(socketPreset("integer").defaultFieldKind).toBe("integer")
    expect(socketPreset("geometry").defaultFieldKind).toBeUndefined()
  })
  ...
  test("provides independent Node, Parameter, Socket and Link renderers", () => {
    const renderers = createNodeRenderers()
    ...
    expect(renderers.parameter).toBe(parameterRenderer)
  ...
  test("delegates one public Parameter exactly once even when it owns two Sockets", () => {
    ...
    const entry = positionNode(node, {x: 20, y: 30, w: 180, h: measureNode(node).height})
    const plan = nodeRenderer.plan({entry, connectedSocketIds: new Set(), selected: false})
    ...
    try {
      ...
      surface.materialize(parent, () => nodeRenderer.render({
  ...
  test("renders canonical INT through public Field without a local Node control", async () => {
    ...
    try {
      ...
      const rect = {x: 20, y: 30, w: 180, h: measureNode(node).height}
      const entry = positionNode(node, rect)
      const plan = nodeRenderer.plan({entry, connectedSocketIds: new Set(), selected: false})
      surface.materialize(parent, () => nodeRenderer.render({
        parameterRenderer: parameterRenderer,
        host: surface,
      ...
      const source = await Bun.file(new URL("./node.ts", import.meta.url)).text()
    ...
    for (const label of ["Mapping", "Noise Texture", "gy"]) {
      for (const collapsed of [false, true]) {
        ...
        try {
          surface.setRect({x: 0, y: 0, w: 240, h: 120}, HEADER_PIXEL_SCALE, projectFont)
          const parent = surface.createParent()
          const node: NodeView = {id: `${label}:${collapsed}`, title: label, collapsed}
          const rect = {x: 20, y: 30, w: 180, h: measureNode(node).height}
          const entry = positionNode(node, rect)
          const plan = planNode(node, rect)
          surface.materialize(parent, () => nodeRenderer.render({
            parameterRenderer: parameterRenderer,
            host: surface,
            entry,
  ...
  test("places loose right sockets above properties and loose left sockets below parameters", () => {
    ...
    const plan = planNode(node, {x: 20, y: 30, w: 240, h: measureNode(node).height})
  ...
  test("uses one calibrated visual envelope for all eight Socket shapes without a scale floor", () => {
    ...
    const expected = socketVisualBounds(center)
    expect(SOCKET_VISUAL_POLICY).toEqual({
      diameter: 10,
      outlineWidth: 1,
      cornerRadius: 1,
      strokeWidth: 2,
    ...
    for (const shape of SOCKET_SHAPES) {
      ...
      expect(Math.max(painted.w, painted.h), `${shape} diameter`).toBeCloseTo(SOCKET_VISUAL_POLICY.diameter)

      for (const parentScale of [0.16, 0.5, 1, 2]) {
        const projectedDiameter = Math.max(painted.w, painted.h) * parentScale
        expect(projectedDiameter, `${shape} @ ${parentScale}`).toBeCloseTo(SOCKET_VISUAL_POLICY.diameter * parentScale)
      }
    }
  })
  ...
  test("keeps Parameter and loose Socket centers on their exact Flex rows and Node borders", () => {
    ...
    const rect = {x: 20, y: 30, w: 240, h: measureNode(node).height}
    const plan = planNode(node, rect)
    ...
    for (const shape of SOCKET_SHAPES.slice(0, 6)) {
      ...
      expect(rect.x - left.x, `${shape} left outside`).toBeCloseTo(SOCKET_VISUAL_POLICY.diameter / 2)
      expect(left.x + left.w - rect.x, `${shape} left inside`).toBeCloseTo(SOCKET_VISUAL_POLICY.diameter / 2)
      expect(rect.x + rect.w - right.x, `${shape} right inside`).toBeCloseTo(SOCKET_VISUAL_POLICY.diameter / 2)
      expect(right.x + right.w - (rect.x + rect.w), `${shape} right outside`).toBeCloseTo(SOCKET_VISUAL_POLICY.diameter / 2)
    }
  })
  ...
  test("anchors Rotation sockets to the label row for input, output, linked and hide-value states", () => {
    for (const state of ["input", "output", "linked", "hidden"] as const) {
      for (const shape of SOCKET_SHAPES) {
        ...
        const entry = positionNode(node, rect)
        const plan = nodeRenderer.plan({entry, connectedSocketIds: connected, selected: false})
  ...
  test("removes only the hidden editor height while preserving label Socket anchor", () => {
    ...
    const unlinkedMeasurement = measureNode(node)
    const frame = {x: 20, y: 30, w: 180, h: unlinkedMeasurement.height}
    const entry = positionNode(node, frame)
    const unlinked = nodeRenderer.plan({entry, connectedSocketIds: new Set(), selected: false})
    const linked = nodeRenderer.plan({entry, connectedSocketIds: new Set(["translation"]), selected: false})
    const hiddenNode: NodeView = {
      ...node,
      sockets: node.sockets!.map((socket) => socket.id === "translation" ? {...socket, hideValue: true} : socket),
    }
    const hiddenEntry = positionNode(hiddenNode, frame)
    const hidden = nodeRenderer.plan({entry: hiddenEntry, connectedSocketIds: new Set(), selected: false})

    expect(unlinkedMeasurement.height).toBe(225)
    expect(measureNode(node, new Set(["translation"])).height).toBe(156)
    ...
    const restored = nodeRenderer.plan({entry, connectedSocketIds: new Set(), selected: false})
    expect(restored.rect).toEqual(unlinked.rect)
    expect(nodeRenderer.presentation?.({entry, connectedSocketIds: new Set(["translation"]), selected: false}, linked)).toEqual({
      ...entry,
      rect: linked.rect,
    ...
    try {
      ...
      const parent = surface.createParent()
      const presentation = nodeRenderer.presentation!({
        entry,
  ...
  test("measures compact default width from intrinsic controls while preserving explicit resize", () => {
    ...
    const unlinked = measureNode(node)
    const linked = measureNode(node, new Set(["translation"]))
    ...
    const defaultPlan = planNode(node, defaultFrame)
    ...
    const resized = planNode(node, {...defaultFrame, w: 240})
    ...
    const longHeader = measureNode({...node, title: longHeaderLabel})
    const longSocket = measureNode({
      ...node,
  ...
  test("keeps preview toggle state separate from the image panel and ordinary Node geometry", () => {
    ...
    const measured = measureNode(base)
    const rect = {x: 20, y: 120, w: 180, h: measured.height}
    const entry = positionNode(base, rect)
    const visible = {overlays: true, previews: true} as const
    const closed = planNode(base, rect, new Set(), visible)
    ...
    const openNode: NodeView = {...base, preview: {...base.preview!, enabled: true}}
    const openEntry = positionNode(openNode, rect)
    const open = planNode(openNode, rect, new Set(), visible)
    ...
    const globalHidden = planNode(openNode, rect, new Set(), {overlays: true, previews: false})
    ...
    expect(planNode(openNode, rect, new Set(), {overlays: false, previews: true}).preview)
...
function paintedSocketBounds(
  ...
  } as unknown as Parameters<typeof socketRenderer.render>[0]["host"]
  const socket: SocketView = {
    id: `socket-${shape}`,
    label: shape,
...
function paintedNodeShadow(collapsed: boolean, selected: boolean): Readonly<{
  ...
  } as unknown as Parameters<typeof nodeRenderer.render>[0]["host"]
...
function paintedNodeHeader(collapsed: boolean): PaintedNodeHeader {
  const node: NodeView = {id: "mapping", title: "Mapping", collapsed}
  const rect = {x: 20, y: 30, w: 180, h: measureNode(node).height}
  const entry = positionNode(node, rect)
  const plan = planNode(node, rect)
