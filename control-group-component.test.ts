...
import {ControlGroup, controlGroupComponentCss} from "./control-group-component.tsx"
...
describe("compiled production ControlGroup", () => {
  ...
  test("keeps one joined contour and class-free native focus ownership", () => {
    ...
    const renderer = createDocumentRenderer({
      ...
      styleSheets: [controlGroupComponentCss]
    })
    ...
    expect(controlGroupComponentCss).toContain(":focus-within")
    expect(controlGroupComponentCss).not.toContain(".ui-")
    renderer.dispose()
    root.unmount()
  })
})
