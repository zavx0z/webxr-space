...
import {Button as runtimeButton} from "./button-component.tsx"
...
beforeAll(async () => {
  outputDirectory = await mkdtemp(join(packageRoot, ".button-component-test-"))
  const result = await Bun.build({
    entrypoints: [join(packageRoot, "button-component.tsx")],
...
describe("compiled production Button", () => {
  ...
  test("compiles the exact public button import for an application consumer", async () => {
    ...
    try {
      const result = await Bun.build({
        entrypoints: [join(packageRoot, "button-component-consumer-fixture.tsx")],
  ...
  test("resolves native hover/active and exact 22px medium geometry", () => {
    ...
    const renderer = createDocumentRenderer({
      ...
      interactionState,
      styleSheets: [compiled.buttonComponentCss]
    })
