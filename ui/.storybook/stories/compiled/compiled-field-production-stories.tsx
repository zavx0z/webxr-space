...
import {ColorPickerField} from "@zavx0z/ui/fields/color-picker-field"
import {CycleField} from "@zavx0z/ui/fields/cycle-field"
import {FieldGroup} from "@zavx0z/ui/fields/field-group"
...
type ToggleButtonGroupProps = Parameters<typeof ToggleButtonGroup>[0]
...
function ToggleButtonGroupStory(props: Readonly<{initial: ToggleButtonGroupProps}>) {
  const [value, setValue] = useState(props.initial.value)
  const onChange = (next: string, event: Event) => {
    setValue(next)
    props.initial.onChange?.(next, event)
...
export function createCompiledToggleButtonGroupProductionStory(document: SemanticDocument, props: ToggleButtonGroupProps) {
  return mountCompiledStory(document, ToggleButtonGroupStory, {initial: props}, "toggle-button-group", componentSource("ToggleButtonGroup", "toggle-button-group", props, "value", "onChange"))
}
...
function componentSource(
  ...
  const ownerSubpath = component === "ToggleButtonGroup"
    ? `buttons/${subpath}`
    : `fields/${subpath}`
  ...
  const iconImport = lines.some(line => line.includes("uiIcons."))
    ...
    : []
  return [
    `import {${component}} from "@zavx0z/ui/${ownerSubpath}"`,
    ...iconImport,
...
function popupSource(
  ...
  const iconImport = lines.some(line => line.includes("uiIcons."))
    ...
    : []
  return [
    `import {${component}} from "@zavx0z/ui/fields/${subpath}"`,
    ...iconImport,
    'import {createRoot, useState} from "@zavx0z/component"',
    "",
    "function Story() {",
