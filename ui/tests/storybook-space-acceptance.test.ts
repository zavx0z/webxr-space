...
test("[UI-ACCEPT-STATIC-002] story consumes the host Space and never creates another Experience", async () => {
  ...
  expect(source, "История не должна требовать Space вместо корневого html").not.toContain("tree.space !== document.documentElement")
  expect(source, "История должна проверять принадлежность Space документу host").toContain("tree.space.ownerDocument !== document")
  ...
  expect(source).toContain("UI display acceptance requires a host-owned @zavx0z/dom/display DisplayElement")
  expect(source).toContain("UI HUD acceptance requires a host-owned @zavx0z/dom/hud HUDElement")
  expect(source, "После представления должна проверяться точная проекция host").toContain("assertProjectionOwner(owner, projection, document, tree)")
  expect(source, "Display должен принадлежать сцене host").toContain("tree.displays.includes(ancestor)")
  expect(source, "HUD должен быть исходным владельцем проекции").toContain("tree.hud?.element === ancestor")
  expect(source, "Показываемый исходник не должен использовать удалённый createExperience").not.toContain("createExperience")
