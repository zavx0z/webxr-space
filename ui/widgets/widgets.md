...
обычного выбора.
Выбор строки указателем сохраняет прокрутку. Клавиатурный переход и явные
`focus`/`reveal` показывают целевую строку при необходимости.
...
`@zavx0z/ui/themes/icons`. Действия над деревом реализует вызывающее приложение.
