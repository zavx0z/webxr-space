...
describe("структурированный разбор TypeScript 7", () => {
  test("реальный DiagramNodeProps: readonly, optional, callback, @property и пример", async () => {
    const result = await analyzeTypeDoc(repository, "nodes/node/diagram/contract/input.ts")
    ...
    expect(declaration.signature).toContain("Readonly<{")
...
export type {Shape as PublicShape} from "./bridge.ts"`,
    ...
    const result = await analyzeTypeDoc(root, "input.ts")
...
export type RecordUnion = {value: string} | {value: number}`})
    const {document} = await analyzeTypeDoc(root, "input.ts")
  ...
  test("не исполняет исходник и импортированный модуль", async () => {
    ...
    expect((await analyzeTypeDoc(root, "input.ts")).document.declarations[0]!.members[0]!.type).toBe("number")
  })
  ...
  test("отклоняет отсутствующий, пустой, синтаксически и семантически сломанный источник", async () => {
    ...
    for (const [name, message] of [
      ...
      const error = await analyzeTypeDoc(root, name).then(() => undefined, error => error)
  ...
  test("отслеживает пустую базу и тип ссылки, но не соседние исходники", async () => {
    ...
    const before = await analyzeTypeDoc(root, "input.ts")
    ...
    const after = await analyzeTypeDoc(root, "input.ts")
  ...
  test("отклоняет сломанный импортированный контракт", async () => {
    ...
    const error = await analyzeTypeDoc(root, "input.ts").then(() => undefined, error => error)
    expect(error).toBeInstanceOf(Error)
