...
import {CodeEditor, codeEditorComponentCss} from "./code-editor-component.tsx"
...
describe("compiled production CodeEditor", () => {
  ...
  test("uses exact editor geometry and class-free token styles", () => {
    ...
    const renderer = createDocumentRenderer({
      ...
      styleSheets: [codeEditorComponentCss]
    })
    ...
    expect(codeEditorComponentCss).not.toContain(".ui-")
  ...
  test("preserves whitespace segment semantics while the renderer emits no whitespace paint", () => {
    ...
    const renderer = createDocumentRenderer({document, root: host, viewport: {width: 640, height: 320}, styleSheets: [codeEditorComponentCss]})
