...
import {layoutFixed, type LayoutGraph} from "@nodes/layout"
import {FixedWorkerClient, runFixedWorkerRequest} from "./index.ts"
...
describe("minimal layout Worker protocol", () => {
  test("structured-clones the synchronous geometry in RIGHT and DOWN", () => {
    for (const viewport of [{width: 900, height: 600}, {width: 390, height: 844}]) {
      ...
      const response = structuredClone(runFixedWorkerRequest({
        type: "layout",
        requestId: 7,
        generation: 3,
        graph: input,
      ...
      if (response.type === "layout-result") {
        expect(response.result).toEqual(layoutFixed(input))
  ...
  test("contains no UI document or text measurements and is permutation-stable", () => {
    ...
    const first = runFixedWorkerRequest({type: "layout", requestId: 1, generation: 1, graph: input})
    const second = runFixedWorkerRequest({type: "layout", requestId: 2, generation: 2, graph: reversed})
  ...
  test("rejects obsolete pending generations without accepting their late response", async () => {
    ...
    const client = new FixedWorkerClient(endpoint as unknown as WorkerEndpoint)
  ...
  test("propagates policy, generation and endpoint failures without a main-thread fallback", async () => {
    ...
    const policyClient = new FixedWorkerClient(policyEndpoint)
    ...
    const mismatchClient = new FixedWorkerClient(mismatchEndpoint)
    ...
    const endpointClient = new FixedWorkerClient(endpointFailure)
    ...
    const throwingClient = new FixedWorkerClient(throwingEndpoint)
...
class FakeWorkerEndpoint {
  ...
  readonly messageListeners = new Set<(event: MessageEvent<WorkerResponse>) => void>()
  ...
  respond(index: number, generation?: number): void {
    const response = runFixedWorkerRequest(this.requests[index]!)
    const delivered = generation === undefined ? response : {...response, generation}
    for (const listener of this.messageListeners) {
      listener({data: structuredClone(delivered)} as MessageEvent<WorkerResponse>)
    }
  }
