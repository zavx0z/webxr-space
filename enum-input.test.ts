...
describe("public EnumInput", () => {
  ...
  test("renders inline expanded choices and publishes only the exact chosen value", () => {
    ...
    expect(compactChrome.map((call) => call[4].fill)).toEqual([
      rgba8ToColor(resolveWidgetColors("toggle").inner),
      rgba8ToColor(resolveWidgetColors("toggle", {selected: true}).inner),
      rgba8ToColor(resolveWidgetColors("toggle").inner),
    ])
    expect(compactChrome.map((call) => call[4].border)).toEqual([
      rgba8ToColor(resolveWidgetColors("toggle").outline),
      rgba8ToColor(resolveWidgetColors("toggle", {selected: true}).outline),
      rgba8ToColor(resolveWidgetColors("toggle").outline),
    ])
  })
