...
import {Color, Mesh, MeshBasicMaterial, TrueTypeFont, type Object3D} from "@engine/core"
...
describe("generic Node Editor contracts", () => {
  test("ports an open Select above later Parameter visuals and preserves it through transform-only Node frames", async () => {
    ...
    const entry = positionNode(node, {x: 40, y: 40, w: 240, h: measured.height})
    const popupTree: PositionedNodeTree<NodeView, SocketView, LinkView, FrameView> = {
    ...
    const fontBytes = await projectFontBytes()
  ...
  test("calls the production intrinsic planner once per dirty Node cycle", async () => {
    const node: NodeView = {
      id: "diagnostic-node",
      title: "Diagnostic",
    ...
    const rect = {x: 20, y: 20, w: 220, h: measureNode(node).height}
    const diagnosticTree: PositionedNodeTree<NodeView, SocketView, LinkView, FrameView> = {
      bounds: rect,
      frames: [],
      nodes: [positionNode(node, rect)],
      links: [],
    }
    const renderers = createNodeRenderers()
    let actualPlans = 0
    let actualParameterRenders = 0
    const canvas = new NodeCanvas<NodeView, SocketView, LinkView, FrameView, NodePlan>({
      renderers: {
        ...renderers,
        node: {
          ...renderers.node,
          plan(context) {
            actualPlans += 1
            return renderers.node.plan(context)
          },
        },
        parameter: {
          render(context) {
            actualParameterRenders += 1
            renderers.parameter.render(context)
          },
        },
      },
      toolbar: false,
    ...
    canvas.setTree({...diagnosticTree, revision: 2, nodes: [positionNode(nextNode, rect)]})
    canvas.flushPendingRender()
