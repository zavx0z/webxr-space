...
import {Badge, badgeComponentCss} from "./badge-component.tsx"
import {Divider, dividerComponentCss} from "./divider-component.tsx"
import {Pane, paneComponentCss} from "./pane-component.tsx"
import {Typography, typographyComponentCss} from "./typography-component.tsx"
...
describe("compiled production visual primitives", () => {
  ...
  test("retains Divider and resolves exact inset geometry", () => {
    ...
    const renderer = createDocumentRenderer({
      ...
      styleSheets: [dividerComponentCss]
    })
  ...
  test("publishes only class-free owner geometry", () => {
    ...
    expect(paneComponentCss).toContain("padding:8px")
    expect(paneComponentCss).toContain("border-radius:4px")
    expect(badgeComponentCss).toContain("min-height:20px")
    expect(badgeComponentCss).toContain("padding:2px 6px")
    expect(typographyComponentCss).toContain("font-size:15px")
    expect(typographyComponentCss).toContain("font-size:11px")
    expect(dividerComponentCss).toContain("height:1px")
    expect(dividerComponentCss).toContain("width:96%")
    expect(dividerComponentCss).toContain("width:90%")
  })
})
