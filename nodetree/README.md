...
`@zavx0z/nodetree` владеет живой моделью графа: Node, Socket, Link, Frame,
...
Визуальное представление модели принадлежит `@zavx0z/nodes`.

## Модель и значения

```typescript
import {Parameter, createNodeTree} from "@zavx0z/nodetree"
...
отмечены в [матрице покрытия](../nodes/storybook-coverage.md). Каталог показывает
