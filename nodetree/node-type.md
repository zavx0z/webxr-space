...
Полный план показа находится в [матрице Storybook](../nodes/storybook-coverage.md).
...
| --- | --- |
| Декларация типа, создание экземпляров и проверка их структуры | `@zavx0z/nodetree` |
| Значения, revisions, подписки, topology и сериализация | Существующие `Parameter` и `NodeTree` |
| Production TSX-представление ноды, сокетов и параметров | `@zavx0z/nodes` |
...
| Раскладка по готовой числовой геометрии | `@zavx0z/layout` |
...
В [node-tree.ts](node-tree.ts) уже есть:
...
- [Parameter](parameter.ts), [NodeTree](node-tree.ts), [foundation](foundation.ts),
  [serialization](serialization.ts), [Nodes projection](../nodes/node-tree.tsx).
