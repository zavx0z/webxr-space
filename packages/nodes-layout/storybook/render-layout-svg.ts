...
export type StorybookGateway = Readonly<{
  edgeId: string
  nodeId: string
  point: LayoutPoint
}>

export type StorybookPortLabel = Readonly<{
  portId: string
  side: "WEST" | "EAST"
  text: string
...
export function layoutPortLabels(result: LayoutResult): readonly StorybookPortLabel[] {
  return (["WEST", "EAST"] as const).flatMap((side) => {
    ...
    return ports.map((port): StorybookPortLabel => {
...
export function findGatewayPoints(graph: LayoutGraph, result: LayoutResult): readonly StorybookGateway[] {
  ...
  const gateways: StorybookGateway[] = []
