...
export type CoreStorybookParameter = Parameter<NodeJsonValue, Readonly<{label: string}>>
export type CoreStorybookTree = NodeTree<CoreStorybookParameter>

export type CoreRuntimeScenario = Readonly<{
  tree: CoreStorybookTree
  changes: readonly NodeTreeChange[]
  setGain(value: number): boolean
  addParameter(): boolean
  removeParameter(): boolean
  snapshot(): NodeTreeSnapshot<CoreStorybookParameter>
  document(): NodeTreeDocument<CoreStorybookParameter>
}>
...
export function createCoreRuntimeScenario(): CoreRuntimeScenario {
  ...
  const tree = new NodeTree<CoreStorybookParameter>({
    nodes: [{
      id: "source",
...
function parameter(id: string, label: string, value: number): CoreStorybookParameter {
