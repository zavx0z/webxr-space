...
describe("package boundary", () => {
  ...
  it("publishes only the runtime and leaves compiler ownership to Template", () => {
    ...
    expect(reactCompatibility.reactPackageAlias).toBe(false)
    expect(reactCompatibility.npmReactDependency).toBe(false)
    expect(reactCompatibility.virtualDom).toBe(false)
    expect(reactCompatibility.fiber).toBe(false)
    expect(reactCompatibility.reactDomHost).toBe(false)
