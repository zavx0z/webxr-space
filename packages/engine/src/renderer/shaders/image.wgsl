...
@binding(1) @group(2) var imageTexture: texture_2d<f32>;
...
fn fs_main(in: VertexOutput) -> @location(0) vec4<f32> {
    ...
    let dims = textureDimensions(imageTexture, 0);
    ...
    let imageAspect = max((f32(dims.x) * vb.z) / max(f32(dims.y) * vb.w, 0.0001), 0.0001);
