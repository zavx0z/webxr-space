...
/** Creates the fixed-policy node projection used by the parent runtime. */
export function createNodeTreeProjector(): NodeTreeProjector<
  RuntimeGeneration,
...
function fixedSocketSides(tree: RuntimeGeneration): ReadonlyMap<string, "left" | "right"> {
  const socketByEndpoint = new Map<string, RuntimeSocket<SocketMetadata>>()
  for (const node of tree.nodes) {
