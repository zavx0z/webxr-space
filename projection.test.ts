...
import {createNodeRenderers} from "./node.ts"
...
): RuntimeParameter => new Parameter<NodeJsonValue, ParameterPresentation>(
  id,
  value,
  {
    label,
...
describe("live NodeTree projection", () => {
  test("measures and lays out once, then reuses geometry for a Parameter-only change", async () => {
    ...
    expect((source as Parameter<NodeJsonValue, ParameterPresentation>).set(2)).toBeTrue()
  ...
  test("reads captured Parameter fields while binding edits to the live Store", async () => {
    const {tree, source} = createTree()
    const projector = createNodeTreeProjector()
    type Input = Parameters<typeof projector.project>[0]
    let release: (() => void) | undefined
    let produced: NodeTreeProjection | undefined
    const delayed = {
      async project(input: Input): Promise<NodeTreeProjection> {
        await new Promise<void>((resolve) => { release = resolve })
        produced = await projector.project(input)
        return produced
      },
    }

    const pending = tree.project(delayed, {
      cacheKey: "reference:captured-parameter",
      context: {viewport: {width: 900, height: 600}},
    })
  ...
  test("remeasures only a Node whose Parameter declares value-sensitive geometry", async () => {
    ...
    ;(source as Parameter<NodeJsonValue, ParameterPresentation>).set(2)
  ...
  test("rejects explicit Socket sides that contradict the fixed projection policy", async () => {
    ...
    await expect(tree.project(createNodeTreeProjector(), {
      cacheKey: "conflict",
      context: {viewport: {width: 900, height: 600}},
