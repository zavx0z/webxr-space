...
describe("compiled boolean controls", () => {
  test("keeps Checkbox live checked/indeterminate state and controlled proposals", () => {
    ...
    const renderer = createDocumentRenderer({
      ...
      styleSheets: [checkboxComponentCss]
    })
  ...
  test("uses one semantic switch button and retains its translated thumb", () => {
    ...
    const renderer = createDocumentRenderer({
      ...
      root: host,
      viewport: {width: 80, height: 50},
      styleSheets: [switcherComponentCss]
    })
  ...
  test("uses class-free native pseudo owner sheets", () => {
    expect(checkboxComponentCss).toContain(":checked")
    expect(checkboxComponentCss).toContain(":indeterminate")
    expect(checkboxComponentCss).not.toContain(".ui-")
    expect(switcherComponentCss).not.toContain(".ui-")
  })
})
