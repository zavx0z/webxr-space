...
describe("compiled production NumberInput", () => {
  ...
  test("keeps hard normalization separate from pointer-only soft bounds", () => {
    ...
    expect(normalizeNumberInputValue(15, options)).toBe(15)
    expect(resolveNumberInputSoftRange(5, options)).toEqual({min: 0, max: 10})
    expect(resolveNumberInputSoftRange(50, {step: 0.01})).toEqual({min: -50, max: 150})
    expect(resolveNumberInputSoftRange(50, {min: 0, max: 100, softMin: 80, softMax: 20})).toEqual({min: 20, max: 80})
    expect(stepNumberInputValue(15, -1, options)).toBe(14.9)
    expect(stepNumberInputValue(15, 1, options)).toBe(10)
    expect(numberInputFillPercentage(5, 0, 10)).toBe(50)
    expect(numberInputFillPercentage(-5, 0, 10)).toBe(0)
    expect(numberInputFillPercentage(15, 0, 10)).toBe(100)
    expect(numberInputFillPercentage(5, undefined, 10)).toBeNull()
    expect(numberInputFillPercentage(5, 10, 10)).toBeNull()
  })
  ...
  test("renders one continuous 120x22 scalar contour", () => {
    ...
    document.appendChild(host)
    const root = createRoot(host)
    root.render(NumberInput as any, {value: 1, min: 0, max: 2})
    const owner = host.querySelector("div")!
