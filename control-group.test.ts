...
import {type UiSurface, UiSurface as BaseUiSurface, Z} from "@layout/core/surface"
import {
  ControlGroup,
...
describe("public ControlGroup", () => {
  ...
  test("maps generic Component roles to source-backed group base, outline and separators", () => {
    for (const [appearance, kind, buttonAppearance] of [
      ...
      expect(surface.roundedRects[0]?.[4].fill).toEqual(rgba8ToColor(colors.inner))
      expect(surface.roundedRects.at(-1)?.[4].border).toEqual(rgba8ToColor(colors.outline))
      expect(surface.roundedRects.find((call) => call[4].radius === 0)?.[4].fill).toEqual(rgba8ToColor(colors.outline))
      expect(context?.buttonAppearance).toBe(buttonAppearance)
      expect(context?.textColor).toEqual(rgba8ToColor(colors.text))
    }
  })
  ...
  test("keeps ordinary group composition on Elements and Flex instead of direct Surface rules", async () => {
    const source = await Bun.file(new URL("./control-group.ts", import.meta.url)).text()
  ...
  test("keeps generic role choices in Components without leaking Blender classes to Node API", async () => {
    const [vector, matrix, path, reference] = await Promise.all([
      Bun.file(new URL("./vector-input.ts", import.meta.url)).text(),
      Bun.file(new URL("./matrix-input.ts", import.meta.url)).text(),
      Bun.file(new URL("./path-input.ts", import.meta.url)).text(),
      Bun.file(new URL("./reference-input.ts", import.meta.url)).text(),
    ])
    ...
    for (const source of [vector, matrix, path, reference]) {
      expect(source).not.toContain("WidgetClass")
