...
const dismissStyle: CssStyle = css`
  & { width: 20px; min-width: 20px; height: 20px; padding: 2px; border: 0 solid transparent; background: transparent; box-shadow: none; }
`
...
export function Notification(props: NotificationProps) {
  ...
  return <aside
    ...
    style={css`
      ...
      &[data-tone="success"] { border-left-color: var(--state-success); }
      &[data-tone="warning"] { border-left-color: var(--state-warning); }
      &[data-tone="error"] { border-left-color: var(--state-error); }
      ${props.style}
    `}
  >
    <span aria-hidden="true" style={css`
      & { display: block; width: 7px; min-width: 7px; height: 7px; margin-top: 5px; border-radius: 4px; background: var(--state-info); }
    ...
    `} data-tone={tone}></span>
    <span style={css`
      & { box-sizing: border-box; display: flex; flex-direction: column; min-width: 0; flex-grow: 1; gap: 1px; }
    `}>
      <strong hidden={props.heading === undefined} style={css`
        & { display: block; font-size: var(--font-size-xs); line-height: 15px; }
        &[hidden] { display: none; }
      `}>{props.heading ?? ""}</strong>
      <span style={css`
        & { display: block; font-size: var(--font-size-xs); line-height: 15px; overflow-wrap: break-word; }
      `}>{props.message}</span>
      <span hidden={props.detail === undefined} style={css`
        & { display: block; color: var(--widget-text-content-readonly); font-size: var(--font-size-2xs); line-height: 14px; overflow-wrap: break-word; }
        &[hidden] { display: none; }
      `}>{props.detail ?? ""}</span>
    </span>
    <span hidden={!dismissible} style={css`
      & { display: block; }
      &[hidden] { display: none; }
    `}>
      <IconButton
