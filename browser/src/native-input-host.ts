...
export type DocumentNativeKeyInput = Readonly<{
  type: "keydown" | "keyup"
  key: string
  code?: string
  location?: number
...
export function createDocumentNativeInputHostWithSeams(
  ...
  const dispatchKey = (
    candidate: DocumentNativeInputTarget,
    input: DocumentNativeKeyInput,
  ): boolean => {
    if (disposed) throw new Error("Document native input host is disposed")
    if (input === null || typeof input !== "object") throw new TypeError("Key input is required")
    ...
    if (proxy === null || target !== candidate || activeTarget(proxy) !== candidate) {
      throw new Error("Semantic key target does not own the active native proxy")
    }
    const accepted = candidate.dispatchEvent(new SemanticKeyboardEvent(input.type, {
      bubbles: true,
      cancelable: true,
      composed: true,
      key: input.key,
    ...
    if (
      ...
      !input.ctrlKey &&
      !input.metaKey &&
      candidate.applyPickerKeyboardDefault(input.key)
    ) {
      options.requestFrame()
    } else if (
      input.type === "keydown" &&
      ...
      !input.ctrlKey &&
      !input.metaKey &&
      candidate.applyRangeKeyboardDefault(input.key)
    ) {
      options.requestFrame()
    } else if (
      input.type === "keydown" &&
