...
Существующая UI-тема загружается автоматически как настоящий CSS asset.
Приложению не нужны theme URL, заранее созданный link или собственный stylesheet
с копией defaults. Явный `theme` заменяет эту тему; `stylesheets` добавляет другие
