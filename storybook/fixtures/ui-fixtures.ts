...
export const SOCKET_CATALOG = SOCKET_KINDS.map((kind, index): SocketView => ({
  id: `catalog-${kind}`,
  label: kind,
...
/** One representative live Node used only for same-scale accepted-reference comparison. */
export function createNoiseComparisonTree(): PositionedNodeTree<NodeView, SocketView, LinkView, FrameView> {
  const noise = nodeView("comparison-noise", "Noise Texture", "Texture", [
  ...
  const nodes: PositionedNode<NodeView, SocketView>[] = [positionCatalogNode(noise, 120, 34, 260)]
  return {
...
export function createCatalogNodeTree(
  ...
  const frame: FrameView = {id: "catalog-frame", label: "Система компонентов нод"}
  const nestedFrame: FrameView = {
    id: "data-frame",
  ...
  const scalar = nodeView("scalar", "Скалярная математика", "Converter", [
    {
      id: "operation",
      label: "Операция",
      kind: "enum",
  ...
  const transform = nodeView("transform", "Преобразование", "Vector", [], [
  ...
  const shader = nodeView("shader", "Principled", "Shader", [
  ...
  const previewableShader = previewNodeIds.includes("shader")
    ...
    : shader
  const asset = nodeView("asset", "Ввод ресурса", "Resource", [
  ...
  const matrix = nodeView("matrix", "Матричная математика", "Utility", [
  ...
  const collapsed: NodeView = {
    ...nodeView("collapsed", "Компактное смешивание", "Converter", [], [], [
  ...
  const nodes: PositionedNode<NodeView, SocketView>[] = [
...
function frameHeightForNodes(
  nodes: readonly PositionedNode<NodeView, SocketView>[],
  frameId: string,
  y: number,
  minimum: number,
): number {
...
function positionCatalogNode(
  ...
  y: number,
  width?: number,
): PositionedNode<NodeView, SocketView> {
  const measurement = measureNode(node)
  return positionNode(node, {x, y, w: width ?? measurement.width, h: measurement.height})
}
...
function socket(
  id: string,
  label: string,
  direction: SocketView["direction"],
  socketType: SocketView["socketType"],
  parameterId?: string,
  side?: SocketView["side"],
): SocketView {
  return {
    id,
    label,
...
): Readonly<{link: LinkView; points: readonly Readonly<{x: number; y: number}>[]}> {
