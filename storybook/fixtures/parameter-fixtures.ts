...
export type ParameterStoryFixture = Readonly<{
  ...
  links: readonly PositionedLink<LinkView>[]
}>
...
const DEMO_SOCKET_KINDS = Object.freeze({
  ...
  path: "string",
  readonly: "custom",
} satisfies Readonly<Record<NodeParameterFieldKind, SocketKind>>)
...
export function createParameterStoryFixture(
  ...
  const editorRect = separateLabel
    ...
    : rect
  const positionedSockets = Object.freeze(sockets.map((socket): PositionedSocket<SocketView> => Object.freeze({
    socket,
...
function parameterSockets(
  ...
  const input: SocketView = Object.freeze({
    id: `${kind}-input`,
  ...
  const output: SocketView = Object.freeze({
    id: `${kind}-output`,
    label: parameter.label,
    direction: "output",
    parameterId: parameter.id,
...
function positionedLinks(
  link: LinkView | null,
  sockets: readonly PositionedSocket<SocketView>[],
): readonly PositionedLink<LinkView>[] {
