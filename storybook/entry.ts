...
async function startComponentsStorybook(): Promise<void> {
  ...
  document.documentElement.dataset.componentsStorybook = "starting"
  document.documentElement.dataset.uiStorybook = "starting"
  document.documentElement.dataset.uiStorybookPage = "components"
  try {
    ...
    document.documentElement.dataset.componentsStorybookPage = "workbench"
    ...
    const publish = (): Readonly<Record<string, unknown>> => {
      ...
      document.documentElement.dataset.componentsStorybookRoute = router.path
      document.documentElement.dataset.componentsStorybookRouteKind = router.node.kind
      ...
      document.documentElement.dataset.componentsStorybookRetained = JSON.stringify(current)
      return current
    }
    ...
    globalThis.__componentsStorybookObserver = Object.freeze({
      snapshot: publish,
...
function publishComponentsError(error: unknown): void {
  document.documentElement.dataset.componentsStorybook = "error"
  document.documentElement.dataset.uiStorybook = "error"
  document.documentElement.dataset.componentsStorybookError = error instanceof Error
    ? error.stack ?? error.message
    : String(error)
}

if (typeof document !== "undefined") await startComponentsStorybook()
