...
import {SOCKET_CATALOG} from "../fixtures/ui-fixtures.ts"

export type SocketCatalogMode = "types" | "shapes" | "states"
export const ACCEPTED_REFERENCE_SRC = storybookPublicPath("node", "/references/blender-4.5.5-reference.png")
...
export class SocketCatalogSurface extends UiSurface {
  ...
  protected override render(): void {
    ...
    const description = this.#mode === "types"
      ...
      : this.#mode === "shapes"
        ? `${SOCKET_SHAPES.length} форм, совместимых с исходным API`
  ...
  #drawShapes(x: number, y: number, w: number, h: number): void {
    ...
    const rows = chunk(SOCKET_SHAPES, columns)
    flexColumn({
      x,
      y,
      w,
      ...
      items: rows.map((row) => ({
        ...
        draw: (rowX: number, rowY: number, rowW: number, rowH: number) => flexRow({
          ...
          items: Array.from({length: columns}, (_, column) => {
            ...
            const socket: SocketView = {id: `shape-${shape}`, label: shape, direction: "bidirectional", socketType: "custom", shape}
  ...
  #drawStates(x: number, y: number, w: number, h: number): void {
    const states: readonly Readonly<{socket: SocketView; label: string; selected: boolean}>[] = [
  ...
  #drawSocketRow(row: readonly SocketView[], columns: number, x: number, y: number, w: number, h: number): void {
    flexRow({
      x,
      y,
      w,
      ...
      items: Array.from({length: columns}, (_, column) => {
        ...
        return socket === undefined
          ...
          : {width: "1fr" as const, height: h, draw: (cellX: number, cellY: number, cellW: number, cellH: number) => this.#drawSocketCell(socket, socketPreset(socket.socketType).label, false, cellX, cellY, cellW, cellH)}
      }),
    })
  }

  #drawSocketCell(socket: SocketView, label: string, selected: boolean, x: number, y: number, w: number, h: number): void {
    ...
    flexRow({
      ...
      alignItems: "center",
      items: [
        {width: 22, height: 22, draw: (slotX, slotY, slotW, slotH) => socketRenderer.render({host: this, entry: {socket, side: socket.direction === "output" ? "right" : "left", center: {x: slotX + slotW / 2, y: slotY + slotH / 2}}, selected, nodeId: "socket-catalog"})},
