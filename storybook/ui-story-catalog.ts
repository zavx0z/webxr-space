...
export const NODE_SOCKET_STORIES = defineStorybookStories({
  groups: [{
    id: "node-components",
  ...
  representative: {component: "socket", section: "boolean", variant: "input"},
})

export const NODE_SOCKET_STORY_ROUTES = Object.freeze(
  [...NODE_SOCKET_STORIES.routeTree.leaves] as NodeSocketStoryRoute[],
)

export const NODE_COMPONENT_STORIES = defineStorybookStories({
  groups: [{
    id: "node-components",
    label: "Компоненты нод",
    components: [
      ...
      },
      {
        ...
        sections: [{
          id: "reference",
          label: "Принятый эталон",
          variants: [{id: "default", label: "Референс", title: "Сравнение с эталоном", load: loadNodeComponentStory("comparison")}],
        }],
      },
    ],
  }],
  representative: {component: "node-editor", section: "scene", variant: "default"},
})

export const NODE_COMPONENT_STORY_ROUTES = Object.freeze(
  [...NODE_COMPONENT_STORIES.routeTree.leaves] as NodeComponentStoryRoute[],
)
...
export function nodeSocketStoryIndex(route: NodeSocketStoryRoute): StorybookStoryIndexItem {
  const story = NODE_SOCKET_STORIES.find(route)
  if (story === undefined) throw new Error(`Unknown Node Socket story: ${route}`)
  return story
}

export function nodeComponentStoryIndex(route: NodeComponentStoryRoute): StorybookStoryIndexItem {
...
): readonly StorybookNavigationItem<NodeComponentStoryRoute>[] {
  const selected = nodeComponentStoryIndex(route)
  const sections = new Map<string, StorybookStoryIndexItem>()
...
): readonly StorybookNavigationItem<NodeSocketStoryRoute>[] {
