...
import {
  NODE_EDITOR_STORYBOOK_BASE_PATH,
  NODE_EDITOR_STORYBOOK_ROUTE,
...
document.documentElement.dataset.nodesStorybookPage = "editor"
document.documentElement.dataset.nodesStorybookRoute = ""

const numberParameter = (id: string, label: string, value: number): RuntimeParameter =>
  new Parameter<NodeJsonValue, ParameterPresentation>(id, value, numberPresentation(id, label))
...
const tree: RuntimeTree = new NodeTree<
  RuntimeParameter,
  FrameMetadata,
  NodeMetadata,
  SocketMetadata,
...
let latestProjection: NodeTreeProjection | null = null
...
export type NodeEditorStorybookObserver = Readonly<{
...
declare global {
  var __nodeEditorStorybookObserver: NodeEditorStorybookObserver | undefined
}
...
try {
  ...
  const route = new StorybookRouteTreeRouter(NODE_EDITOR_STORYBOOK_ROUTE_TREE, {
    basePath: storybookPublicPath("node", NODE_EDITOR_STORYBOOK_BASE_PATH),
  })
  const currentRoute = (): string => route.current.path
  const navigate = (next: string): void => {
    if (!route.go(next)) throw new Error(`Unknown editor storybook route: ${next}`)
  }
  const item: StorybookNavigationItem<string> = {
    id: "node-tree-runtime",
    label: "Живой NodeTree",
    route: NODE_EDITOR_STORYBOOK_ROUTE,
    group: {id: "runtime", label: "Runtime"},
  }
  const backdrop = new StorybookBackdropSurface()
  const catalog = new StorybookNavigationSurface({
    title: "Пакет @nodes/editor",
    items: [item],
    route: currentRoute(),
    onNavigate: (next) => navigate(next),
  })
  const sections = new StorybookNavigationSurface({
    title: "NodeTree",
  ...
  const panelOptions = (): StorybookStoryPanelOptions => ({
    source: [
      ...
      "export const projector = createNodeTreeProjector()",
  ...
  const publish = (): void => {
    ...
    document.documentElement.dataset.nodesStorybookRoute = currentRoute()
    document.documentElement.dataset.nodesStorybookRouteKind = route.current.kind
  ...
  globalThis.__nodeEditorStorybookObserver = Object.freeze({
...
function diagnosticsText(projection: NodeTreeProjection | null, materializations: number): string {
...
function publishError(error: unknown): void {
  ...
  document.documentElement.dataset.nodesStorybookError = message
  console.error(error)
}
