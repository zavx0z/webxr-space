...
describe("@ui/components package-owned Workbench stories", () => {
  ...
  test("catalogs every exact public input leaf as its own component", async () => {
    ...
    const referenceImplementation = await Bun.file(join(storybookRoot, "stories", "reference-input.ts")).text()
    ...
    const collectionImplementation = await Bun.file(join(storybookRoot, "stories", "collection-input.ts")).text()
