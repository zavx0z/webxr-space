...
import {getStorybookPolicy} from "./layout-policies.ts"
import {runStorybookLayout} from "./layout-runner.ts"
import type {StorybookFixture, StorybookRun} from "./layout-storybook-types.ts"
...
let currentRun: StorybookRun | null = null
...
function runEditedInput(): void {
  try {
    const graph = JSON.parse(inputEditor.value) as StorybookFixture["graph"]
    const fixture = getStorybookFixture(fixtureSelect.value)
    const run = runStorybookLayout(fixture.policyId, graph)
    currentRun = run
    singleView.hidden = false
    comparison.hidden = true
    comparison.replaceChildren()
    viewTitle.textContent = `${getStorybookPolicy(run.policyId).label} · ${formatDirection(run.result.direction)}`
...
function compareFixtureFamily(): void {
  try {
    const selected = getStorybookFixture(fixtureSelect.value)
    ...
    const runs = fixtures.map((fixture) => ({
      fixture,
      run: runStorybookLayout(selected.policyId, fixture.graph),
    }))
...
function comparisonArticle(fixture: StorybookFixture, run: StorybookRun): HTMLElement {
...
function resetFixture(): void {
  const fixture = getStorybookFixture(fixtureSelect.value || STORYBOOK_FIXTURES[0]!.id)
...
function updatePolicyPresentation(fixture: StorybookFixture): void {
  const policy = getStorybookPolicy(fixture.policyId)
  policyOutput.value = policy.label
  policyOutput.dataset.policyId = policy.id
  policyDescription.textContent = policy.description
}

function successDiagnostics(run: StorybookRun, fixture: StorybookFixture): unknown {
...
function formatMetrics(run: StorybookRun): string {
