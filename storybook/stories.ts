...
const loadControlGroupStory = async (): Promise<StorybookStoryModule> => {
  const {createControlGroupStory} = await import("./stories/control-group.ts")
  return createControlGroupStory()
}

const loadReferenceInputStory = async (): Promise<StorybookStoryModule> => {
...
export const COMPONENT_STORIES = defineStorybookStories({
  groups: [
    {
      id: "foundation",
      label: "Основные",
...
export function componentVariantItems(
  route: ComponentsStoryRoute,
): readonly StorybookNavigationItem<ComponentsStoryRoute>[] {
