...
describe("final remaining Node DOM route composition", () => {
  ...
  test("owns real Frame, Link, popup and accepted comparison compositions", () => {
    ...
    expect(link.graph.links.every(({route}) => route.kind === "orthogonal" && route.points.slice(1)
      .every((point, index) => point.x === route.points[index]!.x || point.y === route.points[index]!.y))).toBeTrue()
