import {Matrix4, Vector3, type Object3D} from "@engine/core"
import {
...
type RetainedEditor = NodeEditor<NodeView, SocketView, LinkView, FrameView, NodePlan>
...
export type StorybookRetainedSnapshot = Readonly<{
...
export type StorybookRetainedObserver = Readonly<{
  snapshot(): StorybookRetainedSnapshot
  setTransform(transform: NodeCanvasTransform): Readonly<{accepted: boolean; snapshot: StorybookRetainedSnapshot}>
  wheelZoom(): Readonly<{before: NodeCanvasTransform; after: NodeCanvasTransform; snapshot: StorybookRetainedSnapshot}>
  pinchZoom(): Readonly<{before: NodeCanvasTransform; after: NodeCanvasTransform; snapshot: StorybookRetainedSnapshot}>
  ...
  select(selection: NodeEditorSelection): Readonly<{accepted: boolean; snapshot: StorybookRetainedSnapshot}>
  publish(): void
  publishAfterFrame(): void
}>

declare global {
  var __nodeComponentRetainedObserver: StorybookRetainedObserver | undefined
}
...
/** Dev-only observer over the actual NodeEditor engine graph used by the storybook. */
export function createStorybookRetainedObserver(editor: RetainedEditor): StorybookRetainedObserver {
  ...
  const snapshot = (): StorybookRetainedSnapshot => {
  ...
  const observer: StorybookRetainedObserver = Object.freeze({
    snapshot,
    ...
    hitNode(nodeId) {
      ...
      if (entry === undefined) throw new Error(`Unknown storybook Node ${nodeId}`)
