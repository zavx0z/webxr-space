...
export async function waitForReferenceFrame(options: ReferenceReadinessOptions): Promise<void> {
  ...
  while (true) {
    ...
    if (status === "failed") throw new Error("Accepted reference texture failed to materialize")
    if (now() >= deadline) throw new Error(`Accepted reference texture timed out after ${timeoutMs}ms`)
    await wait(pollMs)
  }
