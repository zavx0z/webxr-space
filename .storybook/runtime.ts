...
export const runtime = Object.freeze({
  ...
  create(context: RuntimeContext) {
    ...
    const unmount = (): void => {
      ...
      current = null
      if (previous.element.parentNode !== null) {
        previous.element.parentNode.removeChild(previous.element)
      }
      previous.dispose?.()
    }
...
function ownerStory(value: unknown, route: string): OwnerStoryDescriptor {
  if (value === null || typeof value !== "object") throw new TypeError(`Invalid UI owner story: ${route}`)
  const descriptor = value as Partial<OwnerStoryDescriptor>
  if (descriptor.route !== route || typeof descriptor.create !== "function") {
    throw new TypeError(`UI owner story does not match route: ${route}`)
  }
...
function ownerStoryResult(value: unknown, document: Document): OwnerStoryResult["story"] {
  if (value === null || typeof value !== "object") throw new TypeError("UI owner story returned no result")
