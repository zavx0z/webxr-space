...
export const story_content_left = defineOwnerStory("elements/primitives/span/content/left", async (document) => {
  ...
  return routeStory(createElementDomStory(document, "elements/primitives/span/content/left"))
})

export const story_content_center = defineOwnerStory("elements/primitives/span/content/center", async (document) => {
  const {createElementDomStory} = await import("../helpers/element-dom-story.ts")
  return routeStory(createElementDomStory(document, "elements/primitives/span/content/center"))
})

export const story_content_right = defineOwnerStory("elements/primitives/span/content/right", async (document) => {
  const {createElementDomStory} = await import("../helpers/element-dom-story.ts")
  return routeStory(createElementDomStory(document, "elements/primitives/span/content/right"))
})
