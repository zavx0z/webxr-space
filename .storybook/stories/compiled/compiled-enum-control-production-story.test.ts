...
import {createCompiledEnumControlProductionStory} from "./compiled-enum-control-production-story.tsx"

describe("compiled EnumControl production story", () => {
  test("serializes the controlled option value rather than its visible label", () => {
    const mounted = createCompiledEnumControlProductionStory(createDocument(), {
      value: "output",
  ...
  test("restores cycle header and popup option icons", () => {
    ...
    const mounted = createCompiledEnumControlProductionStory(document, {
      value: "output",
      presentation: "cycle",
