...
} from "@ui/components/controls/integer-control"
import type {Document, Element, Event, HTMLElement, HTMLInputElement, Node} from "@zavx0z/dom"
import {createRoot, useState} from "@zavx0z/react"
import type {RoutedProductionComponentStory} from "../story-types.ts"

function IntegerControlStoryComponent(props: Readonly<{initial: IntegerControlProps}>) {
...
export function createCompiledIntegerControlProductionStory(
  document: Document,
  props: IntegerControlProps
): RoutedProductionComponentStory {
  const staging = document.createElement("div")
  const root = createRoot(staging)
  root.render(IntegerControlStoryComponent as any, {initial: props})
  const owner = staging.querySelector("div") as HTMLElement | null
  if (!owner) {
    root.unmount()
    throw new Error("Compiled IntegerControl story mounted no owner")
  }
  staging.removeChild(owner)
  owner.setAttribute("data-story-component", "integer-control")
...
function source(props: IntegerControlProps, value: number): string {
  return [
