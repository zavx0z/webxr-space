...
function runtimeImportBindings(
  ...
  for (const statement of sourceFile.statements) {
    ...
    if (statement.moduleSpecifier.text !== "@zavx0z/component") continue
    if (statement.importClause?.name) {
      throw compileError(sourcePath, "@zavx0z/component has no default compiler import")
    }
    const named = statement.importClause?.namedBindings
    if (named && !isNamedImports(named)) {
      throw compileError(sourcePath, "@zavx0z/component namespace imports are unsupported")
    }
    if (!named) continue
    for (const specifier of named.elements) {
...
function assertNoReactRuntimeReferences(sourceFile: SourceFile, sourcePath: string): void {
  const reject = (moduleName: string): void => {
    ...
    throw compileError(
      sourcePath,
      "React runtime references are forbidden; import hooks and createRoot from @zavx0z/component",
    )
  }
  visit(sourceFile, node => {
