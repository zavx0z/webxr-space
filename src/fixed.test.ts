...
import * as layoutPackage from "@nodes/layout"
...
describe("fixed layout policy", () => {
  ...
  test("publishes the exact fixed policy at the root entrypoint", () => {
    expect(layoutPackage.layoutFixed(graph())).toEqual(layoutFixed(graph()))
  })
