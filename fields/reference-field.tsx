...
const buttonStyle: CssStyle = css`& { height: 26px; padding: 3px 7px; border: none; border-right: 1px solid var(--widget-regular-outline); border-radius: 0; box-shadow: none; color: var(--widget-regular-content); font-size: 11px; }`
...
export function ReferenceField(props: ReferenceFieldProps) {
  ...
  >
    ...
    <div data-labelled={hasLabel ? "true" : undefined} data-density={density} data-readonly={props.readOnly === true ? "true" : undefined} style={css`
      & { box-sizing: border-box; display: flex; flex-direction: row; min-width: 0; width: 260px; height: var(--control-height-large); gap: 0; padding: 0; overflow: clip; border: var(--border-width-control) solid var(--widget-regular-outline); border-radius: 4px; background: var(--widget-regular-background); box-shadow: 0 1px 0 var(--material-widget-emboss); }
    ...
    `}>
      <Button label={valueLabel} startIcon={resourceIcon} variant="text" title={props.value?.kind ?? props.title} disabled={props.disabled === true || props.onActivate === undefined} style={css`${buttonStyle}${valueStyle}${density === "compact" && compactButtonStyle}`} onClick={props.onActivate} />
      <IconButton label="Choose reference" iconSrc={pickerIcon} title="Choose reference" disabled={props.disabled === true || props.readOnly === true || pickUnavailable} style={css`${buttonStyle}${actionStyle}${density === "compact" && compactButtonStyle}${pickUnavailable && hiddenStyle}`} onClick={props.onPick} />
      <IconButton label="Clear reference" iconSrc={closeIcon} title="Clear reference" disabled={props.disabled === true || props.readOnly === true || clearUnavailable} style={css`${buttonStyle}${actionStyle}${lastActionStyle}${density === "compact" && compactButtonStyle}${clearUnavailable && hiddenStyle}`} onClick={props.onClear} />
    </div>
  </div>
}
