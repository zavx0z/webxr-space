...
describe("@zavx0z/renderer-browser boundary", () => {
  ...
  test("contains no UI, Layout, Storybook, native DOM mirror or hidden font fetch", async () => {
    ...
    expect(spaceSource).toContain("engineRenderer.renderComposition")
    expect(spaceSource).toContain('controls: "host"')
    expect(spaceSource).toContain("addWorld")
    expect(indexSource).toContain("DocumentSpaceWorldRuntime")
