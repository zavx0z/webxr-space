...
export const createDocumentRenderer = (
  ...
  function flush(): RenderFrame {
    if (flushing) throw new Error("Cannot read geometry during Renderer layout")
    flushing = true
    try { return flushFrame() } finally { flushing = false }
  }

  function flushFrame(): RenderFrame {
    assertActive()
  ...
  function forgetSubtree(node: Node): void {
    layoutCache.delete(node)
    for (const child of node.childNodes) forgetSubtree(child)
  }
    ...
    for (const record of batch.records) {
      ...
      if (
        ...
        if (record.type === "childList") {
          // Removed subtrees can change while another projection owns them. Do not
          // reuse their old styles when they later return to the same parent.
          for (const removed of record.removedNodes) forgetSubtree(removed)
        }
...
const placeInlineLayout = (
  ...
  for (const node of layout.owners) {
    ...
    const box: RenderBox = Object.freeze({
      ...createBox(node, bounds.x, bounds.y, bounds.width, bounds.height, ZERO_EDGES, ZERO_BORDER, nodeDepth, "inline", context.presentation,
...
const scrollBox = (box: RenderBox, dx: number, dy: number, owner: RenderTransform): RenderBox => Object.freeze({
  ...box,
  ...(box.fragments === undefined ? {} : {fragments: Object.freeze(box.fragments.map(rect => Object.freeze({...rect, x: rect.x - dx, y: rect.y - dy})))}),
  x: box.x - dx,
  y: box.y - dy,
