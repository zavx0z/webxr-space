...
export type RenderBox = Readonly<{
  /** Individual inline border fragments before CSS transform and viewport projection. */
...
export type CreateDocumentRendererOptions = Readonly<{
  ...
  /** Disable DOM geometry ownership for an auxiliary, non-presented layout calculation. */
  registerGeometry?: boolean | undefined
  /** Browser maps projection-local CSS points to native viewport CSS pixels; absent means this renderer owns the viewport. */
  projectClientPoint?: ((point: Readonly<{x: number; y: number}>) => Readonly<{x: number; y: number}> | null) | undefined
}>
