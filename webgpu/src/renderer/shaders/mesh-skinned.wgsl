...
fn vs_main(
    ...
    worldPosition = perObject.modelMatrix * skinnedPosition;

    let skinnedNormal = (skinMatrix * vec4<f32>(normal, 0.0)).xyz;
    worldNormal = (perObject.normalMatrix * vec4<f32>(skinnedNormal, 0.0)).xyz;
