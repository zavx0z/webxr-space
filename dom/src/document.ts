...
export type DocumentElementFactory = (
  document: Document,
  localName: string,
) => Element
...
export class Document extends Node {
  ...
  createElement(localName: string): Element {
    ...
    if (!(element instanceof Element)) {
      throw new TypeError(`Element factory for ${normalizedLocalName} must return an Element`)
    }
    if (element.ownerDocument !== this) {
      throw new TypeError(`Element factory for ${normalizedLocalName} returned an Element from another Document`)
    }
    if (element.localName !== normalizedLocalName) {
      throw new TypeError(
        `Element factory for ${normalizedLocalName} returned localName ${element.localName}`,
      )
    }
    return element
  }
...
function normalizeDocumentElementFactories(
  options: DocumentOptions,
): ReadonlyMap<string, DocumentElementFactory> {
  if (options === null || typeof options !== "object") {
    throw new TypeError("Document options must be an object")
  }
  const {elementFactories} = options
  if (elementFactories === undefined) return new Map()
  if (
    elementFactories === null ||
    typeof elementFactories !== "object" ||
    Array.isArray(elementFactories)
  ) {
    throw new TypeError("Document elementFactories must be an object")
  }

  const normalized = new Map<string, DocumentElementFactory>()
  for (const [authoredLocalName, factory] of Object.entries(elementFactories)) {
    const localName = authoredLocalName.toLowerCase()
    if (localName.length === 0) {
      throw new TypeError("Document element factory localName cannot be empty")
    }
    if (typeof factory !== "function") {
      throw new TypeError(`Document element factory for ${localName} must be a function`)
    }
    if (normalized.has(localName)) {
      throw new TypeError(`Duplicate Document element factory for ${localName}`)
    }
    normalized.set(localName, factory)
  }
  return normalized
}
