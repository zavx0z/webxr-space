...
import {CollectionControl} from "./collection-control.tsx"
import {uiIcons} from "../icons.ts"
import {createDocument} from "../document.fixture.ts"

describe("compiled production CollectionControl", () => {
  test("composes List and Buttons with retained keyed items and standard actions", () => {
    expect(isCompiledTemplate(CollectionControl)).toBe(true)
    ...
    root.render(CollectionControl as any, {
      items,
      selectedId: "b",
  ...
  test("preserves exact visible-row heights and class-free joined composition", () => {
    ...
    document.appendChild(host)
    const root = createRoot(host)
    root.render(CollectionControl as any, {items: [], selectedId: null, visibleRows: 4, emptyLabel: "Empty"})
