...
export type NumberControlProps = Readonly<{
  value: number
...
export type NumberControlValueOptions = Pick<
  NumberControlProps,
  "numberKind" | "min" | "max" | "softMin" | "softMax" | "step"
>

export type NumberControlSoftRange = Readonly<{min: number; max: number}>
...
type ActiveScrub = Readonly<{
  ...
  lastX: number
  changed: boolean
  dragRange: NumberControlSoftRange
}>
...
export function NumberControl(props: NumberControlProps) {
  ...
  const fillPercentage = numberControlFillPercentage(props.value, props.min, props.max)

  const propose = (value: number, event: Event) => {
    if (!locked && Number.isFinite(value)) props.onInput?.(normalizeNumberControlValue(value, props), event)
  }
  ...
  const onChange = (event: Event) => {
    const value = (event.target as HTMLInputElement).valueAsNumber
    if (!locked && Number.isFinite(value)) props.onChange?.(normalizeNumberControlValue(value, props), event)
  }
  const onFocus = () => {
  ...
  const onPointerDown = (event: PointerEvent) => {
    ...
    const origin = normalizeNumberControlValue(props.value, props)
    ...
    scrub.current = Object.freeze({
      ...
      lastX: event.clientX,
      changed: false,
      dragRange: resolveNumberControlDragRange(origin, props)
    })
  }
  const onPointerMove = (event: PointerEvent) => {
    const active = scrub.current
    if (!active || locked) return
    const rawCurrent = scrubNumberControlRawValue(
      active.rawCurrent,
    ...
    const projected = event.ctrlKey
      ? snapLinearNumberControlValue(rawCurrent, active.dragRange, event.shiftKey)
      : rawCurrent
    const current = normalizeNumberControlValue(projected, props)
    scrub.current = Object.freeze({
      ...active,
      current,
      rawCurrent,
  ...
  const stepAtEdge = (direction: -1 | 1, event: Event) => {
    if (locked) return
    const next = stepNumberControlValue(props.value, direction, props)
...
export function numberControlFillPercentage(
  value: number,
...
export function normalizeNumberControlValue(
  value: number,
  options: NumberControlValueOptions = {}
): number {
...
export function resolveNumberControlSoftRange(
  value: number,
  options: NumberControlValueOptions = {}
): NumberControlSoftRange {
  ...
  const center = normalizeNumberControlValue(value, options)
...
export function scrubNumberControlValue(
  value: number,
  deltaX: number,
  distanceX: number,
  options: NumberControlValueOptions = {},
  ...
  const range = resolveNumberControlDragRange(value, options)
  const raw = scrubNumberControlRawValue(value, deltaX, distanceX, range, options, shift)
  const candidate = ctrl ? snapLinearNumberControlValue(raw, range, shift) : raw
  return normalizeNumberControlValue(candidate, options)
}

/** Applies the reference linear Ctrl snap law for the active frozen soft range. */
export function snapLinearNumberControlValue(
  value: number,
  range: NumberControlSoftRange,
  small = false
): number {
...
export function stepNumberControlValue(
  value: number,
  direction: -1 | 1,
  options: NumberControlValueOptions = {}
): number {
  const range = resolveNumberControlSoftRange(value, options)
  const candidate = normalizeNumberControlValue(value + numberPointerStep(options) * direction, options)
  return direction < 0 ? Math.max(range.min, candidate) : Math.min(range.max, candidate)
}

function scrubNumberControlRawValue(
  value: number,
  deltaX: number,
  distanceX: number,
  range: NumberControlSoftRange,
...
function resolveNumberControlDragRange(
  value: number,
  options: NumberControlValueOptions
): NumberControlSoftRange {
  const range = resolveNumberControlSoftRange(value, options)
...
function numberPointerStep(options: NumberControlValueOptions): number {
  return validStep(options.step) ?? 0.1
}

function numberPointerAdaptiveSpan(options: NumberControlValueOptions): number {
