...
export type VectorControlProps = Readonly<{
...
export function VectorControl(props: VectorControlProps) {
...
function normalizeVector(props: VectorControlProps): Readonly<{
  value: readonly number[]
  axes: readonly string[]
  step: number
}> {
  if (!Array.isArray(props.value) || props.value.length < 2 || props.value.length > 4 || !props.value.every(Number.isFinite)) {
    throw new TypeError("VectorControl value must contain 2 to 4 finite numbers")
  }
  ...
  if (axes.length !== props.value.length || new Set(axes).size !== axes.length || axes.some(axis => typeof axis !== "string" || axis.length === 0)) {
    throw new Error("VectorControl axes must be unique and match value length")
  }
  const step = props.step ?? 0.1
  if (!Number.isFinite(step) || step <= 0) throw new RangeError("VectorControl step must be positive")
