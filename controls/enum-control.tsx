...
export type EnumControlOption = Readonly<{
  key: string
  value: string
  label: string
...
export type EnumControlDensity = "regular" | "compact"
export type EnumControlPresentation = "cycle" | "expanded"
export type EnumControlState = "ready" | "undefined" | "error"

export type EnumControlProps = Readonly<{
  value: string
  options?: readonly EnumControlOption[] | undefined
  presentation?: EnumControlPresentation | undefined
  state?: EnumControlState | undefined
  density?: EnumControlDensity | undefined
  disabled?: boolean | undefined
...
function EnumOption(props: Readonly<{option: EnumControlOption; selected: boolean; hidden: boolean}>) {
  return <option
...
function ExpandedEnumOption(props: Readonly<{
  option: EnumControlOption
  selected: boolean
  locked: boolean
  onChange?: EnumControlProps["onChange"]
}>) {
...
function EnumOptions(props: Readonly<{
  options: readonly EnumControlOption[]
  value: string
...
export function EnumControl(props: EnumControlProps) {
  ...
  const exceptionalLabel = enumControlExceptionalLabel(props)
  ...
  const selected = findEnumControlOption(props.value, options)
...
export function findEnumControlOption(
  value: string,
  options: readonly EnumControlOption[]
): EnumControlOption | undefined {
  return options.find(option => option.value === value)
}

/** Cycles stable option values while preserving the established invalid-value behavior. */
export function nextEnumControlValue(
  value: string,
  options: readonly EnumControlOption[],
  step = 1
): string {
...
function enumControlExceptionalLabel(props: EnumControlProps): string | undefined {
...
function assertEnumProps(props: EnumControlProps): void {
  if (typeof props.value !== "string") throw new TypeError("EnumControl value must be a string")
  if (props.presentation !== undefined && props.presentation !== "cycle" && props.presentation !== "expanded") {
    throw new Error(`Unknown EnumControl presentation: ${props.presentation}`)
  }
  if (props.state !== undefined && props.state !== "ready" && props.state !== "undefined" && props.state !== "error") {
    throw new Error(`Unknown EnumControl state: ${props.state}`)
  }
  if (props.options === undefined) return
  if (!Array.isArray(props.options)) throw new TypeError("EnumControl options must be an array")
  const keys = new Set<string>()
  const values = new Set<string>()
  for (const option of props.options) {
    if (typeof option.key !== "string" || option.key.length === 0) {
      throw new TypeError("EnumControl option key must not be empty")
    }
    if (keys.has(option.key)) throw new Error(`EnumControl option key must be unique: ${option.key}`)
    keys.add(option.key)
    if (typeof option.value !== "string" || typeof option.label !== "string") {
      throw new TypeError("EnumControl option value and label must be strings")
    }
    if (values.has(option.value)) throw new Error(`EnumControl option value must be unique: ${option.value}`)
