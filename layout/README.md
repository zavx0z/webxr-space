...
`@zavx0z/layout` принимает измеренный числовой граф и возвращает геометрию.
...
| --- | --- | --- |
| [Fixed](src/fixed.ts) | Измеренные ноды, `y` портов, связи, viewport, spacing | `RIGHT` / `DOWN`, источник `EAST`, приёмник `WEST`, ортогональные sections. Один порт с конфликтующими ролями отклоняется |
| [Adaptive](src/adaptive.ts) | Тот же граф с `capability` и `allowedSides` | Одна сторона для каждого точного сокета, включая общий. Возвращает bounds, sections и диагностику ограниченного поиска |
| [TopDown](src/top-down.ts) | Плоский DAG и `x` портов без viewport | `SOUTH` → `NORTH`, единые цепочки cubic curves. Цикл возвращает typed witness |
| [Coffman–Graham](src/coffman-graham.ts) | DAG, `x` портов и `maxNodesPerLayer` | Ограниченные по ширине слои, cubic curves и массив crossings. Цикл возвращает typed witness |
...
[LayoutGraph / LayoutResult](types/protocol.ts): measured размеры,
...
У каждого алгоритма есть собственные [client/executor](worker/index.ts).
