...
import {
  ...
  type NodeTreeStore,
  type NodeTreeLayout,
} from "../../node-tree/src/node-tree.tsx"
import type {ParameterInput} from "@nodes/parameters/shared"
import {getNodeTreeLayoutStore, type NodeTreeLayoutState} from "../../shared/projection/layout-state.ts"

/**
Навигация по одному NodeTree с общей принятой геометрией.

@property store - Исходный Store дерева; Editor не создаёт вторую модель.

@property layout - Раскладка или source-bound receipt; до её принятия ввод для устаревшей геометрии заблокирован.

@property [transform] - Управляемое смещение и масштаб; без него Editor хранит навигацию локально.

@property [selection] - Управляемый выбор; изменение передаётся через onSelectionChange.
*/
