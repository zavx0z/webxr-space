...
test("[NODES-LAYOUT-COHERENCE-002] GraphEditor shares accepted geometry for topology, input and fit", async () => {
...
test("[NODES-LAYOUT-COHERENCE-011] value-only changes invalidate an exact receipt until a new receipt is delivered", async () => {
  ...
  try {
    ...
    expect(pending(mounted)).toBe(true)
    expect(mounted.element.querySelector("[data-graph-scene]")?.hasAttribute("hidden")).toBe(true)
...
function pending(mounted: Mounted): boolean {
  return mounted.element.querySelector("[data-graph-view]")?.getAttribute("data-layout-pending") === "true"
}

function fit(mounted: Mounted): Element {
  const result = mounted.element.querySelector('button[aria-label="Вписать"]')
  if (result === null) throw new Error("Missing GraphEditor fit control")
  return result
}
