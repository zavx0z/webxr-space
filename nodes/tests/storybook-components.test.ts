...
test("[NODES-CATALOG-005] Node, Frame, Link, NodeTree and NodeEditor variants mount production owners", async () => {
  const selectors: Record<string, string> = {node: "article[data-node-id]", frame: "[data-frame-id]", link: "[data-link-id]", "node-tree": "[data-node-tree]", "node-editor": "[data-node-editor]"}
