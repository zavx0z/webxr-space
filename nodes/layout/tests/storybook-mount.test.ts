...
Bun.plugin(createTemplateJsxBunPlugin({cwd: workspace, persistent: true, sourceRoots: [resolve(workspace, "nodes/layout"), resolve(workspace, "ui")]}))
...
test("[LAYOUT-STORYBOOK-MOUNT] числовой сценарий использует Document и Display host, повторный расчёт и cleanup", async () => {
  ...
  const session = runtime.create({
    ...
    present(value) {
      ...
      expect(value.source.typescript).toContain('from "@nodes/layout/top-down"')
    },
