/**
JSON Patch с атомарным отказом над переносимыми данными графа.

Операции не изменяют входной JSON; malformed пути, неподдерживаемые значения
и превышение лимитов отклоняются до публикации результата.
