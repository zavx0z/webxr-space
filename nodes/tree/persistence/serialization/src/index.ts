/**
Проверка, сериализация и восстановление переносимого документа.

Сериализуются order/byId и значения; подписки и вычисленные проекции не
передаются. Hydration создаёт отдельный канонический Store после валидации.

@packageDocumentation
*/
import {
  type Ordered,
} from "../../../model/node-tree/src/index.ts"
import {
  createNodeTree,
  createValidatedParameter,
  type FoundationNodeTreeOptions,
} from "../../../shared/foundation.ts"
import type {NodeTree} from "../../../model/node-tree/src/index.ts"
import {
  Parameter,
  type NodeJsonValue,
} from "../../../model/parameter/src/index.ts"
import {applyJsonPatch} from "../../json-patch/src/index.ts"
import {
  requirePortableNodeTreeDocument,
  type PortableNodeTreeDocument,
} from "../../../model/node-tree/src/document.ts"
...
export type {PortableNodeTreeDocument} from "../../../model/node-tree/src/document.ts"
